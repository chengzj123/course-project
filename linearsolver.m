function [Ur,In]=linearsolver(a,c,Tol,C,GL,GR,s,PhiL,PhiR,ui,F,r)
         ...
         for i=1:Num
             ...
             Z=zeros(10,1);
             for j=1:10
                 G=integral(@(x) w(x),0,K(j)*r^(-0.5))/...
                     integral(@(x) w(x),0,r^(-0.5));
                 Z(j)=(Mat(4,10*(i-1)+j+1)-G)^2;
             end
             In=In+LIntegral(Y(1),Y(2),Y(2),Z);
         end        
end
